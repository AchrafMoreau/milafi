...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // after submmition the form this function should run if everythig is okey
    const afterSubmitEvent = ()=>{
    ...
    const submitForm = (e)=>{
        ...
        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            ...
            const type = form.elements['category'].value;
            const title = form.elements['title'].value;
            const location = form.elements['event-location'].value;
            const eventDay = form.elements['event-day'].value;
            ...
            if (forms[0].checkValidity() === false) {
                ...
                $.ajax({
                    url: '/events', 
                    method: 'POST',
                    data: {type, title, location, eventDay, startTime, endTime, description},
                    constactType: 'application/json',
                    headers:{
                        'X-CSRF-TOKEN': token
                    },
